...
object 二维数组查找指定元素_01 {
     ...
     * 如果该数字大于要查找的数字，剔除这个数字所在的行：如果该数字小于要查找
     * 的数字，剔除这个数字所在的列。也就是说如果要查找的数字不在数组的右上角，
     * 则每－次都在数组的查找范围中剔除行或者一列，这样每一步都可
     * 以缩小查找的范围，直到找到要查找的数字，或者查找范围为空。
     */
    private fun find(matrix: Array<IntArray>?, number: Int): Boolean {
